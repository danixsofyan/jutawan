...
class M_home extends CI_Model
{
    ...
    {
        $this->db->select('lokasi.id, nama_lks, kategori, alamat, no_tlp, website, photo, nama_ktrg, icon, SUM(jumlah) AS jumlah_total');
        $this->db->from('lokasi');
        $this->db->join('kategori', 'kategori.id = lokasi.kategori');
        $this->db->join('data_pengunjung', 'data_pengunjung.id_lokasi = lokasi.id');
        $this->db->group_by('lokasi.id, nama_lks, kategori, alamat, no_tlp, website, photo, nama_ktrg');
